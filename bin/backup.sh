...
case "$1" in
    ...
    "mysql")
        if [[ -n "$(dockerContainerId mysql)" ]]; then
            ...
            dockerExecMySQL sh -c "MYSQL_PWD=\"${MYSQL_ROOT_PASSWORD}\" mysqldump -hlocalhost -uroot --opt --single-transaction --events ${MYSQL_DATABASE} --routines --comments" |  gzip > "${BACKUP_DIR}/${BACKUP_MYSQL_FILE}"
            logMsg "Finished"
        else
