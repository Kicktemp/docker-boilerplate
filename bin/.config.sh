...
PUBLIC_DIR=$($READLINK -f "$ROOT_DIR/public")

BACKUP_DIR=$($READLINK -f "$ROOT_DIR/private")
BACKUP_SOLR_FILE='solr.cores.tbz2'
BACKUP_MYSQL_FILE='current.sql.gz'
MYSQL_DATABASE='db'
...
dockerExecApplication() {
    docker exec -i --user application "$(docker-compose ps -q app)" "$@"
}
